...
serve(async (req) => {
  ...
  try {
    ...
    if (!order.doctor_email) {
      return new Response(JSON.stringify({ ok: false, error: "No provider assigned to this order" }), { status: 400, headers: { ...CORS, "Content-Type": "application/json" } });
    }
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0">
                      <tr>
                        <td style="padding:3px 0;">
                          ...
                          <strong style="font-size:13px;color:${ACCENT};">${patientFullName}</strong>
                        </td>
                      </tr>
                      <tr>
                        <td style="padding:3px 0;">
                          <span style="font-size:12px;color:#6b7280;width:110px;display:inline-block;">Order ID</span>
                          <strong style="font-size:13px;color:${ACCENT};font-family:monospace;">${confirmationId}</strong>
                        </td>
                      </tr>
                      <tr>
                        <td style="padding:3px 0;">
                          <span style="font-size:12px;color:#6b7280;width:110px;display:inline-block;">State</span>
                          <strong style="font-size:13px;color:${ACCENT};">${stateName}</strong>
                        </td>
                      </tr>
                    </table>
                  </td>
