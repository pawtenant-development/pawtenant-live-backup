      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:580px;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e5e7eb;">
        <tr>
          <td style="background:${HEADER_BG};padding:32px;text-align:center;">
            ...
            <div style="display:inline-block;background:${HEADER_BADGE_BG};color:${HEADER_TEXT};font-size:11px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;padding:5px 16px;border-radius:999px;margin-bottom:14px;">Application Update</div>
            <h1 style="margin:0;font-size:22px;font-weight:800;color:${HEADER_TEXT};line-height:1.3;">Still reviewing your application</h1>
            <p style="margin:10px 0 0;font-size:14px;color:${HEADER_SUB};line-height:1.5;">We haven&apos;t forgotten about you, ${escapeHtml(firstName)}!</p>
          </td>
        </tr>
        <tr>
          <td style="padding:36px 32px;">
                ...
                <table width="100%" cellpadding="0" cellspacing="0">
                  <tr>
                    <td style="padding:8px 0;vertical-align:top;width:32px;">
                      <div style="width:24px;height:24px;background:${ACCENT};border-radius:50%;text-align:center;line-height:24px;font-size:11px;font-weight:700;color:#fff;">1</div>
                    </td>
            ...
            <p style="margin:0;font-size:14px;">
              <a href="mailto:${SUPPORT_EMAIL}" style="color:${ACCENT};font-weight:700;text-decoration:none;">${SUPPORT_EMAIL}</a>
            </p>
          </td>
        </tr>
        <tr>
          <td style="background:#f9fafb;border-top:1px solid #f3f4f6;padding:20px 32px;text-align:center;">
            <p style="margin:0 0 4px;font-size:12px;color:#9ca3af;">${COMPANY_NAME} &mdash; Provider Network &mdash; <a href="https://${COMPANY_DOMAIN}" style="color:${ACCENT};text-decoration:none;">${COMPANY_DOMAIN}</a></p>
...
async function sendViaResend(to: string, subject: string, html: string): Promise<{ sent: boolean; error?: string }> {
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: FROM_ADDRESS, to: [to], subject, html, reply_to: SUPPORT_EMAIL, tags: [{ name: "email_type", value: "provider_application_followup" }] }),
    });
    if (!res.ok) { const err = await res.text(); return { sent: false, error: `Resend ${res.status}: ${err}` }; }
    return { sent: true };
  ...
  try {
    ...
    if (body.bulk) {
      ...
      const { data: pending, error: fetchErr } = await adminClient
        ...
        .eq("status", "pending").lt("created_at", cutoff).is("followup_sent_at", null);
      ...
      for (const app of pending) {
        ...
        const { sent, error } = await sendViaResend(email, `Still reviewing your ${COMPANY_NAME} provider application`, buildProviderFollowUpHtml(firstName, email));
        results.push({ email, sent, error });
        if (sent) await adminClient.from("provider_applications").update({ followup_sent_at: new Date().toISOString() }).eq("id", app.id);
      }
    ...
    const { sent, error } = await sendViaResend(body.email, `Still reviewing your ${COMPANY_NAME} provider application`, buildProviderFollowUpHtml(firstName, body.email));

    if (!sent) return json({ ok: false, error: error ?? "Failed to send email" }, 500);

    await adminClient.from("provider_applications").update({ followup_sent_at: new Date().toISOString() }).eq("email", body.email.toLowerCase()).eq("status", "pending");
