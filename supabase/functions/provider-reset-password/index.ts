...
serve(async (req) => {
  ...
  try {
    ...
    if (!authUser) {
      return new Response(JSON.stringify({ ok: true, message: "If an account exists with that email, a reset link has been sent." }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const { data: profile } = await adminClient.from("doctor_profiles").select("full_name, is_admin").eq("user_id", authUser.id).maybeSingle();
...
<body style="margin:0;padding:0;background:#f3f4f6;font-family:Arial,Helvetica,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f3f4f6;padding:40px 20px;">
    ...
    const resendRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: "PawTenant <hello@pawtenant.com>", to: [email], subject: "Reset your PawTenant provider password", html: htmlBody }),
    });
    ...
    return new Response(JSON.stringify({ ok: true, message: `Reset link sent to ${email}. Please check your inbox (and spam folder).` }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
