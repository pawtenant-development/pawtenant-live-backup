...
serve(async (req) => {
  ...
  try {
    ...
    const { data: callerProfile } = await adminClient.from("doctor_profiles").select("role, is_admin").eq("user_id", callerUser.id).maybeSingle();
    ...
    if (userId) {
      ...
      if (getUserErr || !authUserResult?.user) {
        return new Response(JSON.stringify({ ok: false, error: `No Supabase Auth account found for user_id "${userId}". Try "Resend Portal Invite" first.` }), { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      const realAuthEmail = authUserResult.user.email ?? "";
      if (realAuthEmail && realAuthEmail.toLowerCase() !== targetEmail) targetEmail = realAuthEmail;
      const { data: providerProfile } = await adminClient.from("doctor_profiles").select("full_name").eq("user_id", userId).maybeSingle();
      if (providerProfile?.full_name) providerName = providerProfile.full_name.split(" ")[0];
    }

    if (!targetEmail) {
      return new Response(JSON.stringify({ ok: false, error: "No email address available for this provider." }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (linkErr) {
      ...
      const errMsg = isNotFound
        ? `No Supabase Auth account found for "${targetEmail}". Use "Resend Portal Invite" to create their account first.`
    ...
    if (!actionLink) {
      return new Response(JSON.stringify({ ok: false, error: "Reset link was not generated. Please try again." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
...
<html>
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Reset Your Password — PawTenant</title></head>
    ...
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;border:1px solid #e5e7eb;overflow:hidden;max-width:560px;width:100%;">
        <tr>
          <td style="background:${HEADER_BG};padding:28px 32px;text-align:center;">
            <img src="${LOGO_URL}" alt="PawTenant" height="36" style="height:36px;display:block;margin:0 auto;" />
            <p style="color:${HEADER_SUB};font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;margin:10px 0 0 0;">Provider Portal</p>
          </td>
        </tr>
        <tr>
          <td style="padding:36px 32px;">
            ...
            <div style="text-align:center;margin:28px 0;">
              <a href="${actionLink}" style="display:inline-block;background:${ACCENT};color:#ffffff;font-size:14px;font-weight:800;text-decoration:none;padding:14px 36px;border-radius:10px;letter-spacing:0.3px;">
                Set New Password
              </a>
            </div>
            <p style="color:#9ca3af;font-size:12px;line-height:1.6;margin:24px 0 0 0;padding-top:20px;border-top:1px solid #f3f4f6;">
              This link expires in <strong>1 hour</strong> and can only be used once. If you did not expect this email, you can safely ignore it.<br/><br/>
        ...
        <tr>
          <td style="background:#f9fafb;border-top:1px solid #f3f4f6;padding:20px 32px;text-align:center;">
            <p style="color:#9ca3af;font-size:11px;margin:0;">
              PawTenant &mdash; ESA Letter Services &mdash; <a href="https://pawtenant.com" style="color:${ACCENT};text-decoration:none;">pawtenant.com</a>
            </p>
          </td>
        </tr>
      </table>
      ...
      try {
        const resendRes = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({ from: "PawTenant <hello@pawtenant.com>", to: [targetEmail], subject: "Reset your PawTenant provider password", html: htmlBody }),
        });
        if (resendRes.ok) emailSentViaResend = true;
        else { const resendErr = await resendRes.text(); console.error("Resend send failed:", resendErr); }
      } catch (resendEx) { console.error("Resend exception:", resendEx); }
    }
