...
Deno.serve(async (req: Request) => {
  ...
  if (action === "update_amount") {
    ...
    const pc = parsePetCount(body.petCount);
    if (pc === null) {
      return json({ error: "petCount must be 1, 2 or 3" }, 400);
    }
  ...
  try {
    ...
    if (letterType !== "psd-consultation" && !isBundle) {
      const lock = await resolveLegacyQuoteLock(confirmationId, baseAmount, petCount);
    ...
    // Record the BASE the server just decided on (pre-discount - the lock has
    // always been about the base, and a coupon must be re-validated on every
    // attempt rather than baked into a future resume). A later resume then
    // reproduces this exact amount instead of silently repricing.
