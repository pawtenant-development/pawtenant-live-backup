...
interface Props {
  ...
  /**
   * Optional LATER TOUCH (ATTRIBUTION-SOURCE-IMMUTABILITY-001). The primary
   * classification above is the ORIGINAL acquisition (first touch); this is a
   * genuinely later touch shown separately — it never replaces the primary.
   */
  laterTouch?: LaterTouchView | null;
}
      ...
      >
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="min-w-0">
            <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Original acquisition</p>
        ...
        {/* ── Later touch (never the acquisition source) ─────────────── */}
        {laterTouch && (
          <>
            <p className="mt-3 mb-1.5 text-[10px] font-bold uppercase tracking-widest text-gray-400">
              Later touch
            </p>
            <div className="text-xs text-gray-700 bg-amber-50/60 border border-amber-100 rounded-md px-3 py-2 space-y-1.5">
              <div className="flex items-center gap-2 flex-wrap">
                {(() => {
                  const lv = visualForAcquisition(laterTouch.classification.label);
                  return (
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md border text-[11px] font-semibold ${lv.color}`}>
                      <i className={lv.icon} />
                      {lv.label}
                    </span>
                  );
              ...
              <Row label="Seen"    value={laterTouch.captured_at ? new Date(laterTouch.captured_at).toLocaleString() : "—"} />
              <Row label="Landing" value={pathOnly(laterTouch.landing_url)} />
              <p className="text-[11px] text-gray-600">{laterTouch.classification.reasoning}</p>
              {laterTouch.click_ids_suppressed && (
                <p className="text-[11px] text-amber-700">
                  <i className="ri-shield-check-line mr-1" aria-hidden="true" />
                  Stored click IDs were present but not proven to be a fresh ad
                  click, so this touch is not counted as paid.
                </p>
              )}
            </div>
          </>
