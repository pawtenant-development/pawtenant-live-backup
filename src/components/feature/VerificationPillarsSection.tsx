...
const pillars = [
  {
    ...
    accent: "from-[#4A8472]/10 to-[#4A8472]/0",
  },
  {
...
export default function VerificationPillarsSection({
  ...
  return (
    <section
      className={`${isCompact ? "py-12 md:py-16" : "py-16 md:py-20"} ${className || "bg-white"}`}
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
          {pillars.map((card) => (
            <div
              ...
              } hover:border-[#4A8472]/40 transition-colors`}
            >
              <div
                className={`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${card.accent} rounded-t-2xl`}
              />
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 flex items-center justify-center bg-[#4A8472]/15 rounded-xl flex-shrink-0">
                  <i className={`${card.icon} text-[#4A8472] text-xl`}></i>
                </div>
                <div className="min-w-0">
                  ...
                  <ul className="hidden sm:block space-y-1.5">
                    {card.points.map((p) => (
                      ...
                      >
                        <i className="ri-checkbox-circle-fill text-[#4A8472] text-sm"></i>
                        {p}
                      </li>
                    ))}
                  </ul>
          ...
          <div className="mt-9 flex flex-col items-center gap-4">
            {showCTA && (
              <Link
                ...
                className="whitespace-nowrap inline-flex items-center gap-2 px-6 py-3 bg-[#4A8472] text-white text-sm font-bold rounded-md hover:bg-[#3F7061] transition-colors cursor-pointer"
              >
            ...
            {showPrivacyNote && (
              <p className="text-xs text-gray-500 max-w-md text-center leading-relaxed">
                <i className="ri-lock-line text-[#4A8472] mr-1"></i>
