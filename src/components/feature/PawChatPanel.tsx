/**
 * PawChatPanel — the HEAVY public chat panel.
 *
 * PageSpeed Phase 6 (2026-05-26): this file used to live inside
 * PawChatWidget.tsx along with the launcher button. The launcher button
 * was pulling in this entire panel + usePawChatThread + ChatAttachmentView
 * + chatAttachments lib on mount, which fired get_visitor_chat_thread and
 * get_visitor_chat_attachments BEFORE any user interaction and shipped
 * the heavy chunk into the LCP-window network tree.
 *
 * The launcher button now lives in PawChatLauncher.tsx (tiny, no thread
 * hook, no Supabase RPC code on first paint). PawChatLauncher only
 * imports THIS file via React.lazy after the user clicks the bubble —
 * which is the only path that fetches the heavy chunk and runs the
 * thread / attachment RPCs.
 *
 * Behavior is otherwise unchanged:
 *   - Sends via the existing capture-chat edge function.
 *   - Reads agent replies via the get_visitor_chat_thread RPC (panel-open
 *     only — see usePawChatThread's panel-open polling).
 *   - Identity capture + attachments unchanged.
 *   - Hidden on admin / portal routes (the launcher already enforces
 *     this, but the prefix check is kept here too for defense in depth).
 */
...
import { markFirstMessage } from "../../lib/visitorSession";
import {
...
interface PawChatPanelProps {
  onClose: () => void;
}

export default function PawChatPanel({ onClose }: PawChatPanelProps) {
  const { pathname } = useLocation();
  const isHidden = HIDDEN_ROUTE_PREFIXES.some((p) => pathname.startsWith(p));

  // Panel is always "open" while mounted — the launcher unmounts this
  // component when the visitor closes the chat.
  const {
    sessionId,
    messages,
    ...
    markSeen,
    maxLen,
  } = usePawChatThread(true);

  useEffect(() => {
    if (IS_DEV) console.debug("[PawChat] panel mounted — chat thread RPC fired");
  }, []);
  ...
  const showIdentityPrompt =
    identityStatus === "pending" && hasVisitorMessage;

  useEffect(() => {
    const el = scrollRef.current;
    if (el) el.scrollTop = el.scrollHeight;
  }, [messages, attachmentsByMessage, showIdentityPrompt]);
