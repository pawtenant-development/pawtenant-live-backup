      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col items-center gap-5">
            ...
            {outlets.map((outlet) => (
              ...
              >
                <div className="w-6 h-6 flex items-center justify-center">
                  <i className={`${outlet.icon} text-gray-500 text-lg`}></i>
                </div>
                <div>
                  <p className="text-sm font-black uppercase tracking-wide text-gray-700 leading-none">{outlet.name}</p>
                  <p className="text-[10px] text-gray-400 leading-none mt-0.5">{outlet.subtext}</p>
                </div>
              </div>
            ))}
          </div>
          <div className="flex flex-wrap items-center justify-center gap-6 mt-1">
            {[
              { icon: "ri-shield-check-fill", label: "HIPAA Compliant", color: "text-emerald-600" },
              { icon: "ri-award-fill", label: "Licensed Professionals", color: "text-orange-500" },
              { icon: "ri-verified-badge-fill", label: "BBB Accredited", color: "text-amber-500" },
              { icon: "ri-lock-fill", label: "SSL Encrypted", color: "text-gray-600" },
            ].map((badge) => (
