  ...
  const handleApply = async () => {
    ...
    try {
      ...
      if (data.valid && data.discount != null) {
        setApplied({ code: code.trim().toUpperCase(), discount: data.discount });
      ...
      <div className="flex items-center gap-2">
        <div className="flex-1 relative min-w-0">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2.5 text-sm border border-gray-200 rounded-xl bg-white focus:outline-none focus:border-orange-400 transition-colors placeholder-gray-400"
          />
        </div>
        <button type="button" onClick={handleApply} disabled={loading || !code.trim()}
          className={`whitespace-nowrap px-4 py-2.5 text-xs font-extrabold rounded-xl transition-colors flex-shrink-0 ${
            loading || !code.trim() ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-orange-500 text-white hover:bg-orange-600 cursor-pointer"
          }`}>
          {loading ? <i className="ri-loader-4-line animate-spin"></i> : "Apply"}
        </button>
      </div>
      {error && <p className="text-xs text-red-500 ml-1 flex items-center gap-1"><i className="ri-error-warning-line"></i>{error}</p>}
    </div>
  );
}
...
function SecurePaymentCard({
  ...
  const handleBeforeSubmit = () => { onBeforeRedirect?.(); return true; };
    ...
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
      ...
      <div className="bg-[#1A5C4F] px-4 sm:px-5 py-3.5 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <p className="text-sm font-extrabold text-white tracking-tight">Secure Payment</p>
        </div>
        {/* Card brand icons */}
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <div className="h-6 px-1.5 flex items-center justify-center bg-white rounded" style={{ minWidth: "32px" }}>
            <span style={{ color: "#1a1f71", fontSize: "10px", fontWeight: 900, fontStyle: "italic" }}>VISA</span>
          </div>
          <div className="h-6 flex items-center justify-center bg-white rounded relative overflow-hidden" style={{ width: "38px" }}>
            <div style={{ position: "absolute", left: "4px", width: "18px", height: "18px", borderRadius: "50%", background: "#eb001b" }}></div>
            <div style={{ position: "absolute", left: "16px", width: "18px", height: "18px", borderRadius: "50%", background: "#f79e1b", opacity: 0.9 }}></div>
          </div>
          <div className="h-6 px-1.5 flex items-center justify-center rounded" style={{ background: "#006fcf", minWidth: "32px" }}>
      ...
      <div className="flex border-b border-gray-100 bg-gray-50/50">
        <button type="button" onClick={() => setActiveTab("card")}
          className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-bold transition-colors cursor-pointer whitespace-nowrap ${
            activeTab === "card" ? "text-orange-500 border-b-2 border-orange-500 bg-white" : "text-gray-400 hover:text-gray-600"
          }`}>
          <i className="ri-bank-card-line text-sm"></i>Card
        </button>
        {!isSubscription && (
          <button type="button" onClick={() => setActiveTab("klarna")}
            className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-bold transition-colors cursor-pointer whitespace-nowrap ${
        ...
        <button type="button" onClick={() => setActiveTab("qr")}
          className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-bold transition-colors cursor-pointer whitespace-nowrap ${
            activeTab === "qr" ? "text-orange-500 border-b-2 border-orange-500 bg-white" : "text-gray-400 hover:text-gray-600"
          }`}>
              ...
              <div className="mx-4 sm:mx-5 my-5 border-2 border-dashed border-gray-200 rounded-xl bg-gray-50 flex flex-col items-center justify-center py-8 px-4 text-center gap-3">
                {stripeSecretError ? (
                  <>
                    ...
                    <div>
                      <p className="text-sm font-bold text-red-700">Payment setup failed</p>
                      <p className="text-xs text-red-500 mt-1 leading-relaxed max-w-xs">{stripeSecretError}</p>
                    </div>
                    <button type="button" onClick={onRetryClientSecret}
                    ...
                    <p className="text-xs text-gray-400">Still having trouble?{" "}
                      <a href="tel:+14099655885" className="font-semibold text-orange-500 hover:underline">Call 409-965-5885</a>
                    </p>
                  </>
                ) : (
                  <>
      ...
      {activeTab === "klarna" && !isSubscription && (
        <>
          {couponSlot}
          <KlarnaPaymentTab
            amount={totalPrice} plan={plan} petCount={petCount} deliverySpeed={deliverySpeed}
            email={email} firstName={firstName} lastName={lastName} state={state}
            agreed={klarnaAgreed} setAgreed={setKlarnaAgreed}
            agreedError={klarnaAgreedError} setAgreedError={setKlarnaAgreedError}
            confirmationId={confirmationId} onSuccess={() => onPaymentSuccess?.("klarna-success")}
      ...
      {activeTab === "qr" && (
        <>
          ...
          <QRPaymentTab
            amount={totalPrice} plan={plan} petCount={petCount} deliverySpeed={deliverySpeed}
            email={email} firstName={firstName} lastName={lastName} state={state}
            agreed={qrAgreed} setAgreed={setQrAgreed}
            agreedError={qrAgreedError} setAgreedError={setQrAgreedError}
            confirmationId={confirmationId} onSuccess={() => onPaymentSuccess?.("qr-success")}
            couponCode={appliedCouponCode}
          />
        </>
      )}
      ...
      <div className="bg-[#F0FDF4] border-t border-[#b8ddd5] px-4 sm:px-5 py-3 flex items-center gap-2">
        ...
        <p className="text-xs text-gray-600">
          Prefer to call?{" "}
          <a href="tel:+14099655885" className="font-bold underline cursor-pointer hover:text-[#1A5C4F]">409-965-5885</a>
          {" "}— complete by phone.
        </p>
      </div>
    </div>
...
export default function Step3Checkout({
  ...
  // Build subscription params for StripeCardForm lazy flow
  const subscriptionParams: SubscriptionParams = {
    petCount: resolvedPetCount,
    deliverySpeed: resolvedDelivery,
    email: step2.email,
    ...
    <div>
      ...
      <div className="lg:hidden mb-4 bg-[#1A5C4F] rounded-2xl px-4 py-3.5 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-[10px] text-white/70 font-bold uppercase tracking-wider">ESA Letter Package</p>
            <p className="text-xs text-white/80 mt-0.5 truncate">{deliveryLabel}</p>
          </div>
        </div>
        <div className="flex-shrink-0 text-right">
          {couponDiscount > 0 && (
            <p className="text-[10px] text-white/60 line-through">${priceBeforeDiscount}.00</p>
          )}
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-5 lg:items-start gap-4 lg:gap-8">
          ...
          <div className="lg:sticky lg:top-28 space-y-3 lg:space-y-4">
            ...
            <div className="hidden lg:block bg-white rounded-2xl border border-gray-200 overflow-hidden">
              <div className="bg-[#1A5C4F] px-5 py-4">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-[10px] text-white/60 font-bold uppercase tracking-wider mb-0.5">Order Summary</p>
                    <p className="text-sm font-extrabold text-white leading-snug">Clinical Evaluation + ESA Documentation</p>
                  </div>
              ...
              <div className="bg-[#1A5C4F] px-5 py-4 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-[10px] text-white/60 font-bold uppercase tracking-wider">Amount Due Today</p>
                ...
                <div className="text-right flex-shrink-0">
                  ...
                  {selectedPlan === "subscription" && <p className="text-[10px] text-white/60 mt-0.5">per year</p>}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Choose Your Plan</p>
                <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-2 py-0.5 rounded-full whitespace-nowrap">Save ${basePrice - subPrice}/yr</span>
              </div>
              <div className="p-3 space-y-2">
                {/* Subscription */}
                <button type="button" onClick={() => onChange({ ...data, plan: "subscription" })}
                  className={`w-full text-left rounded-xl border-2 px-3 py-3 transition-all cursor-pointer ${
                    selectedPlan === "subscription" ? "border-orange-400 bg-[#FFF7ED]" : "border-gray-200 bg-white hover:border-gray-300"
                  }`}>
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                        selectedPlan === "subscription" ? "border-orange-500" : "border-gray-300"
                      }`}>
                        {selectedPlan === "subscription" && <div className="w-2 h-2 rounded-full bg-orange-500"></div>}
                      </div>
                      <div className="min-w-0">
                        <span className="text-sm font-extrabold text-gray-800 block">Subscribe &amp; Save</span>
                        <span className="text-xs text-gray-500">Auto-renews yearly · Cancel anytime</span>
                      </div>
                    </div>
                    <div className="flex-shrink-0 text-right">
                      <span className="text-xs bg-emerald-100 text-emerald-700 font-bold px-1.5 py-0.5 rounded-full block mb-0.5 whitespace-nowrap">BEST VALUE</span>
                      <span className="text-sm font-extrabold text-orange-500">${subPrice}.00</span>
                    </div>
                  </div>
                </button>

                {/* One-time */}
                <button type="button" onClick={() => onChange({ ...data, plan: "one-time" })}
                  className={`w-full text-left rounded-xl border-2 px-3 py-3 transition-all cursor-pointer ${
                    selectedPlan === "one-time" ? "border-orange-400 bg-[#FFF7ED]" : "border-gray-200 bg-white hover:border-gray-300"
                  }`}>
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                        selectedPlan === "one-time" ? "border-orange-500" : "border-gray-300"
                      }`}>
                        {selectedPlan === "one-time" && <div className="w-2 h-2 rounded-full bg-orange-500"></div>}
                      </div>
                      <div className="min-w-0">
                        <span className="text-sm font-extrabold text-gray-800 block">One-time Purchase</span>
                        <span className="text-xs text-gray-500">Single payment · Klarna available</span>
                      </div>
                    </div>
                    <span className="text-sm font-extrabold text-gray-700 flex-shrink-0">${basePrice}.00</span>
                  </div>
                </button>
              </div>
            </div>
            ...
            {/* ── Klarna badge ── */}
            <div className="bg-[#fff0f5] border border-[#f9c6d8] rounded-2xl px-4 py-3">
              <div className="flex items-center gap-2.5">
                <div className="w-14 h-7 flex items-center justify-center bg-[#ffb3c7] rounded-lg text-xs font-extrabold text-[#17120e] flex-shrink-0">Klarna</div>
                <div className="min-w-0">
                  <p className="text-xs font-bold text-gray-800">4 interest-free payments of ${(totalPrice / 4).toFixed(2)}</p>
                  <p className="text-[10px] text-gray-500">No credit impact · Instant approval</p>
                </div>
              </div>
            </div>
            ...
            {/* ── SMS Consent ── */}
            <label className="flex items-start gap-2.5 cursor-pointer bg-white rounded-xl border border-gray-100 px-4 py-3 hover:border-gray-200 transition-colors">
              <input
                type="checkbox"
                checked={data.smsConsent ?? false}
                onChange={(e) => onChange({ ...data, smsConsent: e.target.checked })}
                className="mt-0.5 accent-orange-500 flex-shrink-0 cursor-pointer"
              />
              <span className="text-xs text-gray-600 leading-relaxed">
                <span className="font-semibold text-gray-700">(Optional)</span> I consent to receive automated text messages from PawTenant. Reply <strong>STOP</strong> to unsubscribe.{" "}
                <a href="/privacy-policy" target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="text-orange-500 font-semibold hover:underline cursor-pointer">Privacy Policy</a>{" "}&amp;{" "}
                <a href="/terms-of-use" target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="text-orange-500 font-semibold hover:underline cursor-pointer">Terms</a>.
              </span>
            </label>
        ...
        {/* ════ LEFT COLUMN — order-2 = second on mobile ════ */}
        <div className="order-2 lg:order-1 lg:col-start-1 lg:col-span-3 flex flex-col gap-4 lg:gap-5">
            ...
            <div className="bg-[#1A5C4F] px-4 sm:px-5 py-4">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-white/70 mt-0.5">100% money-back if you don&apos;t qualify</p>
                </div>
              </div>
            </div>
            <div className="px-4 sm:px-5 py-4 border-b border-gray-100">
              ...
              <div className="mt-3">
                <Link to="/ESA-letter-verification"
                  ...
                  <i className="ri-external-link-line text-xs"></i>How verification works
                </Link>
              </div>
            </div>
            <div className="bg-[#F0FDF4] border-t border-[#b8ddd5] px-4 sm:px-5 py-3 flex items-center gap-2">
          ...
          {/* ESA Document Preview — hidden on mobile to keep checkout focused */}
          <div className="hidden sm:block bg-white rounded-2xl border border-gray-200 overflow-hidden">
            <div className="px-5 pt-5 pb-3">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">What You&apos;ll Receive</p>
              <p className="text-sm font-extrabold text-gray-900">Your Official ESA Letter</p>
            </div>
            <div className="w-full overflow-hidden">
              <img
                src="https://storage.readdy-site.link/project_files/dfb46e5c-44ab-4c6d-87e4-adaf8c9bc491/8496bb5f-3256-4901-86f7-84bbb2ec3596_PawTenant-ESA-document-with-callouts.png?v=56e54caa565cc010fb19c7679d66a2b4"
                alt="PawTenant ESA Letter sample document with callouts showing key features"
          ...
          {/* Mobile-only: compact review strip instead of video */}
          <div className="sm:hidden bg-white rounded-2xl border border-gray-200 px-4 py-4">
            ...
            <div className="space-y-2.5">
              {[
                { name: "Sarah M.", state: "CA", text: "Got my letter in under 24 hours. My landlord accepted it immediately!" },
                { name: "James T.", state: "TX", text: "Super easy process. The provider was professional and thorough." },
                { name: "Maria L.", state: "FL", text: "Finally found a service I can trust. Completely changed my housing situation." },
              ].map((r) => (
      ...
      <div className="mt-8 sm:mt-14">
        <div className="text-center mb-6 sm:mb-8">
          ...
          <h3 className="text-lg sm:text-xl font-extrabold text-gray-900">What Happens Next</h3>
        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-4">
            {[
              { num: "01", icon: "ri-file-check-line", color: "bg-amber-500", title: "Assessment Submitted", desc: "Your evaluation answers are immediately available to a licensed provider in your state." },
              { num: "02", icon: "ri-stethoscope-line", color: "bg-orange-500", title: "Licensed Provider Review", desc: "A board-licensed mental health professional evaluates your case within your chosen delivery window." },
              { num: "03", icon: "ri-mail-check-line", color: "bg-orange-500", title: "ESA Letter Delivered", desc: "Your signed, official ESA letter is emailed as a professional PDF document — ready to use." },
              { num: "04", icon: "ri-home-heart-line", color: "bg-emerald-500", title: "Use for Housing", desc: "Present your letter with full Fair Housing Act protection for rentals, vacation homes, or college housing." },
            ].map((step) => (
              <div key={step.num} className="flex lg:flex-col items-start lg:items-center gap-3 lg:gap-0 relative z-10">
                <div className={`w-12 h-12 flex-shrink-0 flex items-center justify-center ${step.color} rounded-2xl lg:mb-4`}>
