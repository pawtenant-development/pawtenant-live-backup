...
/**
 * Per-employee compensation history + add form (Employee HR Directory detail
 * panel, admin-gated by the parent). Reads the table directly (admin RLS);
 * writes go through the request_compensation_adjustment RPC, which mirrors
 * pending rows into the Manager Approvals inbox. Owner/admin/finance may
 * approve immediately via the auto-approve toggle (enforced server-side).
 */
    ...
    <div className="mb-4">
      ...
      {rows === null ? (
        ...
        <div className="rounded-lg border border-gray-200 overflow-hidden">
          {rows.map((r) => (
            <div key={r.id} className="flex items-center justify-between gap-2 px-3 py-2 border-b border-gray-50 last:border-b-0">
      ...
      <p className="mt-1.5 text-[10px] text-gray-400">
        ...
        in the Manager Approvals inbox; rejected rows never count.
      </p>
    </div>
  );
}
