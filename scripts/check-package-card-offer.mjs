...
const OFFER = [
  ...
  { payable: 149, compareAt: 179, savings: 30, installment: "37.25" }, // ESA/PSD exactly 3 animals
...
async function main() {
  ...
  // ESA and PSD Standard one-time both cover up to two animals at $129 and
  // reserve the fixed $149 total for exactly three.
  if (typeof getPackageTotal === "function") {
    const expect = {
      esa_standard: { 1: 129, 2: 129, 3: 149 },
      psd_standard: { 1: 129, 2: 129, 3: 149 },
    };
    ...
    // ESA and PSD agree at every valid count.
    for (const n of [1, 2, 3]) {
    ...
    // Two-animal pricing is explicitly the same for both products.
    ...
    if (p2 !== 129) problems.push(`PSD standard one_time (2) must be 129, got ${p2}`);
    if (e2 !== p2) problems.push(`ESA/PSD one_time must agree at 2 animals; got ${e2} vs ${p2}`);
  }
