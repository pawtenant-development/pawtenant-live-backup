...
| **P0** | `GOOGLE-ADS-PRIMARY-PURCHASE-CHANNEL-GATE-001-LIVE-PROMOTION` | Primary purchase channel gate + fail-closed invocation authorization → LIVE | ADS | ✅ `LIVE COMPLETE` (closeout in the task doc) | LIVE | ported from TEST `aa34a84`+`17820e3` only | `sync-google-ads-conversions` v87→v88 (`verify_jwt=true` kept), `channelGate.ts`, `invocationAuth.ts`, `adminFunctionAuth.ts`, Google Ads admin panels, Sync Health | LIVE base `c46450e6`; 643 paid → 338 eligible / 297 excluded / 8 conflict; 264 historical non-Google uploads left untouched | 🔴 Only canonically Google Ads purchases reach the Primary action; the PUBLIC anon key is refused (it was a valid project JWT and every admin call site sent it). LIVE had no `lib.ts`/refund consumer ⇒ `cleanClickId` inlined, unhandled known modes refused instead of defaulting to backfill. Guards 180 + 296 checks, 14/14 + 16/16 controls. `docs/tasks/GOOGLE-ADS-PRIMARY-PURCHASE-CHANNEL-GATE-001-LIVE-PROMOTION.md` | ✅ |
